...
def build_partner_demo_pack(output_dir: Path) -> dict[str, Any]:
    ...
    (output_dir / "pilot-summary.md").write_text(_render_pilot_summary(manifest), encoding="utf-8")
    manifest = _build_manifest(output_dir, instruction, approved, execution, audit, video)
...
def _render_pilot_summary(manifest: dict[str, Any]) -> str:
    instruction = manifest["instruction"]
    workflow = manifest["workflow"]
    video = manifest["video"]
    return f"""# Procedra pilot result summary

## Status

This is a synthetic controlled-demo summary generated by `make partner-demo-pack`.
It is not customer validation, production approval, safety certification, or a
measured business-impact report.

## What was tested

- One manufacturing instruction scenario using synthetic demonstration data.
- Structured instruction generation with deterministic demo configuration.
- Source/context retrieval with {instruction['source_count']} sources, including
  {instruction['public_source_count']} public references.
- Quality evaluation with score {instruction['overall_score']}/100 and risk
  level `{instruction['risk_level']}`.
- Markdown, JSON, and PDF artifact export.
- Version save, expert-review transition, approval transition, audit trail, and
  trial execution evidence.
- Synthetic fallback video processing with {video['keyframe_count']} keyframes
  and {video['segment_count']} semantic stages.

## What was not tested

- Real customer deployment or live production use.
- Legal, safety, regulatory, or standards compliance.
- Customer-confidential documents, personal data, or regulated data.
- Expert-labeled correctness against an approved local instruction.
- Measured productivity, time savings, safety improvement, or error reduction.
- Integration with ERP, MES, EAM, LMS, SSO, document-control, or other enterprise systems.
- Internet-facing production hardening, backup/retention operations, or multi-host deployment.

## Evidence included in this pack

- `01-request.json` - synthetic input request.
- `02-instruction.json` and `03-instruction.md` - generated instruction artifacts.
- `04-instruction.pdf` - exported PDF.
- `05-lifecycle.json` - saved version and workflow transitions.
- `06-execution.json` - trial execution evidence with
  {workflow['execution_completed_steps']}/{workflow['execution_total_steps']} steps completed.
- `07-audit.json` - audit events ({workflow['audit_event_count']} events).
- `08-fallback-demo-video.mp4`, `09-video-keyframes.json`, and `keyframes/` -
  synthetic video fixture and extracted keyframes.
- `10-video-instruction.json` - instruction generated from video-derived context.
- `manifest.json` - artifact hashes and metadata.
- `talk-track.md` - suggested seven-minute walkthrough.

## Suggested next pilot step

Run one narrow controlled pilot with:

- one approved non-confidential instruction family;
- one small approved document set;
- named reviewer roles;
- explicit success criteria before the pilot starts;
- a written separation between what was tested, what was not tested, and what
  requires expert or legal review.

Do not use the generated draft for real-world operational work unless qualified
customer reviewers verify and approve it under the customer's own procedures.
"""
