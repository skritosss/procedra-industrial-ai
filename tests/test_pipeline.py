...
def test_pipeline_falls_back_when_openai_fails(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        pipeline,
        "get_settings",
        lambda: SimpleNamespace(openai_enabled=True, openai_api_key="present", openai_model="test-model", openai_timeout_seconds=1, llm_base_url=None, llm_allow_external_calls=True),
    )
...
def test_pipeline_uses_fallback_when_openai_disabled(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        pipeline,
        "get_settings",
        lambda: SimpleNamespace(openai_enabled=False, openai_api_key="present", openai_model="test-model", openai_timeout_seconds=1, llm_base_url=None, llm_allow_external_calls=True),
    )
...
def test_poisoned_retrieved_source_fails_closed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda: SimpleNamespace(
            ...
            openai_timeout_seconds=1, llm_base_url=None, llm_allow_external_calls=True,
        ),
    )
